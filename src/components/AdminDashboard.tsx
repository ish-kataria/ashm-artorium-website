...
export default function AdminDashboard() {
  ...
  const [selectedItem, setSelectedItem] = useState<{type: string, item: Artwork | Class | Event | Order | Customer | Booking} | null>(null);
  ...
  const handleDeleteItem = (type: keyof AdminData, id: number) => {
    setAdminData(prev => ({
      ...prev,
      [type]: (prev[type as keyof AdminData] as (Artwork | Class | Event | Order | Customer | Booking)[]).filter((item) => item.id !== id)
    }));
  };

  const handleEditItem = (type: string, item: Artwork | Class | Event | Order | Customer | Booking) => {
    setSelectedItem({ type, item });
    if (type === 'artwork' && 'title' in item && 'price' in item) {
      setArtworkForm({
        title: item.title,
  ...
  const handleUpdateItem = () => {
    ...
    let updatedItem: Artwork | Class | Event | Order | Customer | Booking;
    ...
    setAdminData(prev => ({
      ...prev,
      [type]: (prev[type as keyof AdminData] as (Artwork | Class | Event | Order | Customer | Booking)[]).map((i) => i.id === item.id ? updatedItem : i)
    }));
                  ...
                  <div className="space-y-3">
                    ...
                    {[...adminData.artwork.slice(-2), ...adminData.classes.slice(-2)].map((item, index) => (
                      <div key={index} className="flex items-center gap-3 p-2 bg-gray-50 rounded">
                        ...
                        <span className="text-sm">{'title' in item ? item.title : 'name' in item ? item.name : 'Item'} added</span>
                      </div>
                    ))}
                  </div>
                )}
